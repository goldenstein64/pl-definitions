...
do
	...
	function lexer.expecting(tok, expected_type, no_skip_ws)
		...
		if t ~= expected_type then
			error("expecting " .. expected_type, 2)
		end
		return v
	end
end
...
local importCompatDiff = {
	start = 1,
	finish = 0,
	text = [[---Raise a warning message.
---
---This function mimics the `warn` function added in Lua 5.4.
---
---Calling `warn("@on")` and `warn("@off")` will turn on and off warnings 
---respectively.
...
]] .. importCompatDiff.text,
}

local hasPlComment, hasPlCompatComment
do
	...
	local function commentPatternsMatcher(name)
		local patterns = {}
		for i, pattern in ipairs(commentPatterns) do
			patterns[i] = string.format(pattern, name)
		end

		return function(tokenValue)
			for _, pattern in ipairs(patterns) do
				if tokenValue:find(pattern) then
					return true
				end
			end
	...
	hasPlComment = commentPatternsMatcher("pl")
	hasPlCompatComment = commentPatternsMatcher("pl%.compat")
end
...
function OnSetText(uri, text)
	...
	local requiredDiff = nil
	for tokenType, tokenValue in lexer.lua(text, { space = true }, { string = true }) do
		if requireState == 0 and tokenType == "iden" and tokenValue == "require" then
			requireState = 1
		elseif requireState == 1 and tokenType == "(" and tokenValue == "(" then
			requireState = 2
		elseif requireState == 2 and tokenType == "string" and requireMap[tokenValue] then
			requiredDiff = requireMap[tokenValue]
			requireState = 3
		elseif requireState == 3 and tokenType == ")" and tokenValue == ")" then
			if requiredDiff == importDiff then
				return { importDiff }
			elseif requiredDiff and not diffSet[requiredDiff] then
				diffSet[requiredDiff] = true
				table.insert(diffs, requiredDiff)
				requiredDiff = nil
				requireState = 0
			end
		elseif tokenType == "comment" then
			if hasPlComment(tokenValue) then
				return { importDiff }
			elseif hasPlCompatComment(tokenValue) and not diffSet[importCompatDiff] then
