local app = require("pl.app") --[[@as pl.app]]
local utils = require("pl.utils")
local path = require("pl.path") --[[@as pl.path]]
...
do -- app.require_here
	...
	if not path.isabs(self) then
		self = path.join(cd, self)
	end
...
do -- app.appfile
	...
	asserteq(stderr, "")
	stdout = path.normcase(stdout)
	local fname = path.normcase(assert(path.expanduser("~/.justsomescriptname_for_penlight_testing/filename.data")))
